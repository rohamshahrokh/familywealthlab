...
/**
 * Crypto ledger server actions.
 * Writes to `ledger.crypto`. Snapshot pulls market-value into accessible wealth.
 */
...
const CryptoSchema = z.object({
  ...
  symbol:          requiredText("Symbol", 1, 20),
...
export async function createCrypto(_prev: unknown, formData: FormData) {
  const session = await requireUser();
  const parsed = CryptoSchema.safeParse(formToObject(formData));
  if (!parsed.success) {
    console.error("[crypto.create] zod", JSON.stringify(parsed.error.issues, null, 2));
    return { ok: false, error: formatZodError(parsed.error) };
  }
  const input = { ...parsed.data, symbol: parsed.data.symbol.toUpperCase() };
  const supabase = createSupabaseServerClient();
  const { data, error } = await supabase.schema("ledger").from("crypto")
    .insert(input).select("id").single();
  if (error) {
    console.error("[crypto.create] supabase", { code: (error as any).code, message: error.message });
  ...
  await logLedgerChange(supabase, {
    ...
    table_name: "crypto", row_id: data.id, action: "insert", diff: input,
  });
  await refreshSnapshotCache(input.household_id).catch(() => undefined);
  revalidatePath(`/workspace/${input.household_id}/input/crypto`);
  revalidatePath(`/workspace/${input.household_id}/overview`);
  return { ok: true };
}

export async function deleteCrypto(formData: FormData): Promise<void> {
  const session = await requireUser();
  const householdId = String(formData.get("household_id") ?? "");
  const id = String(formData.get("id") ?? "");
  if (!householdId || !id) return;
  const supabase = createSupabaseServerClient();
  const { error } = await supabase.schema("ledger").from("crypto")
    .delete().eq("id", id).eq("household_id", householdId);
  if (error) {
    console.error("[crypto.delete] supabase", error);
    return;
  }
  await logLedgerChange(supabase, {
    household_id: householdId, user_id: session.user.id,
    table_name: "crypto", row_id: id, action: "delete", diff: {},
  });
  await refreshSnapshotCache(householdId).catch(() => undefined);
  revalidatePath(`/workspace/${householdId}/input/crypto`);
