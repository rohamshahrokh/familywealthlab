...
import { formatCurrency, safeNum, projectInvestment, calcCAGR } from "@/lib/finance-port/finance";
...
import { runCashEngine } from "@/lib/finance-port/cashEngine";
import SaveButton from "@/components/port/SaveButton";
...
import { fetchAllStockPrices, formatLastUpdated, isStaleByAge, type PriceEntry } from "@/lib/finance-port/marketData";
import { Input } from "@/components/ui/input";
import type { StockTransaction, StockDCASchedule } from "@/lib/finance-port/localStore";
import {
...
// fetchAllStockPrices is imported from @/lib/finance-port/marketData (Yahoo→Stooq fallback)
...
export default function StocksPage() {
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      {(() => {
        const holdingsTotal = (stocks ?? []).reduce(
          (s: number, x: any) => s + safeNum(x.current_price) * safeNum(x.current_holding),
          0,
        );
        const watchlistRows = (stocks ?? []).length;
        if (holdingsTotal === 0 && watchlistRows > 0) {
          return (
            <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-3 text-xs text-amber-700">
              <span className="font-semibold">{watchlistRows} tickers on watchlist · 0 holdings</span> ·
        ...
        <>
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              {/* Audit P1-9: render an explicit empty state when there are
                  no holdings, rather than a flat zero-line chart that
                  reads as broken. */}
              {totalCurrentValue <= 0 ? (
                <EmptyState
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={combinedProjection} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="value" stroke="hsl(var(--v2-chart-3))" fill="url(#stockGrad)" strokeWidth={2} name="Portfolio Value" />
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              {allocationData.length > 0 ? (
                ...
                /* Audit P1-9: unified empty state instead of an ad-hoc inline block. */
                <EmptyState
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={combinedProjection} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 10, fill: "hsl(220,10%,55%)" }} tickFormatter={v => `$${(v / 1000).toFixed(0)}K`} />
                  <Tooltip content={<CustomTooltip />} />
                  <Area type="monotone" dataKey="value" stroke="hsl(var(--v2-chart-3))" fill="url(#projGrad2)" strokeWidth={2} name="Portfolio Value" />
                  <Area type="monotone" dataKey="invested" stroke="hsl(var(--v2-chart-1))" fill="none" strokeWidth={1.5} strokeDasharray="4 2" name="Total Invested" />
                </AreaChart>
              </ResponsiveContainer>
