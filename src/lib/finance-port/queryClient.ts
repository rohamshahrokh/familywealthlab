...
function isDemoMode(): boolean {
  // COMMERCIAL PORT: Force demo mode for the commercial app until a real backend
  // is wired. Every /api/* call is served from the in-memory demo store.
  // To restore conditional demo mode, replace this body with the original
  // localStorage check from `fwl-original/client/src/lib/queryClient.ts`.
  return true;
}
